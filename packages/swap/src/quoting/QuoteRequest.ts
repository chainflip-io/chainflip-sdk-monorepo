...
export default class QuoteRequest {
  ...
  private async getPoolQuote({
    ...
    let swapRateResult = await getSwapRateV3({
      ...
      dcaParams,
      excludeFees,
      includeInternalSwapNetworkFee: isAtLeastSpecVersion('1.10') && this.isOnChain, // this will return correct network fee in v1.10
    });
    ...
    if (this.isOnChain && !isAtLeastSpecVersion('1.10')) {// TODO: check the version and do adjustments if v < 1.10
