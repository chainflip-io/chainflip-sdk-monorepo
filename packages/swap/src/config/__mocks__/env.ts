export default {
  RPC_NODE_HTTP_URL: 'http://rpc-node.test',
  START_HTTP_SERVICE: false,
  START_PROCESSOR: false,
  SWAPPING_APP_PORT: 8080,
  BITCOIN_DEPOSIT_TRACKER_URL: '',
  RPC_BROKER_HTTPS_URL: 'https://rpc-broker.test',
  RPC_NODE_WSS_URL: 'ws://rpc-node.test',
  CHAINFLIP_NETWORK: 'perseverance',
  QUOTE_TIMEOUT: 1000,
  NODE_ENV: 'test',
  CHAINALYSIS_API_KEY: undefined,
  INDEXER_DATABASE_URL: 'postgresql://postgres:postgres@localhost:5432/squid_archive',
  INDEXER_QUERY_TIMEOUT: 10_000,
  PROCESSOR_BATCH_SIZE: 50,
  PROCESSOR_TRANSACTION_TIMEOUT: 10_000,
  REDIS_URL: 'redis://localhost:6379',
  REDIS_URL_BACKOFFICE: 'redis://localhost:6380',
  MAINTENANCE_MODE: false,
  FULLY_DISABLED_INTERNAL_ASSETS: new Set(),
  DISABLED_DEPOSIT_INTERNAL_ASSETS: new Set(),
  DISABLED_DESTINATION_INTERNAL_ASSETS: new Set(),
  MAX_CHANNELS_OPEN_PER_ADDRESS: 10,
  DISABLE_QUOTING: false,
  QUOTING_ALLOWED_ORIGINS: new Set(),
  SOLANA_RPC_HTTP_URL: 'http://solana-rpc.test',
  BITCOIN_RPC_HTTP_URL: 'http://bitcoin-rpc.test',
  STABLE_COIN_SLIPPAGE_MIN_PRICE: 0.995,
  DCA_CHUNK_INTERVAL_BLOCKS: 2,
  QUOTER_BALANCE_TOLERANCE_PERCENT: 10,
  QUOTING_BASE_SLIPPAGE: {},
  QUOTER_DCA_V2_DEPOSIT_ASSETS: new Set(),
  QUOTER_DCA_V2_DESTINATION_ASSETS: new Set(),
  QUOTER_DCA_V2_MAX_USD_VALUE: undefined,
  QUOTER_USE_MEV_FACTOR: true,
  BROKER_COMMISSION_BPS: 0,
  SOLANA_TX_REF_QUEUE_INTERVAL: 500,
  DCA_DISABLED_INTERNAL_ASSETS: new Set(),
  DCA_DEFAULT_SELL_CHUNK_SIZE_USD: 10000,
  DEFAULT_TIGHT_LPP_SLIPPAGE_BPS: 50,
  DEFAULT_LPP_SLIPPAGE_BPS: undefined,
  CHECK_ALL_LPS_FOR_LIQUIDITY: false,
  MESSAGE_QUEUE_DELAY_MS: 0,
  ENABLE_QUOTE_MESSAGE_QUEUE: false,
  ENABLE_QUOTE_RATE_LIMIT: true,
  QUOTE_RATE_LIMIT_WINDOW_MS: 60_000,
  QUOTE_RATE_LIMIT_MAX_REQUESTS: 100,
  QUOTE_RATE_LIMIT_BLOCK_DURATION_MS: 300_000,
  ENABLE_NOT_FOUND_RATE_LIMIT: true,
  NOT_FOUND_MAX_REQUESTS_LIMIT: 10,
  NOT_FOUND_RATE_LIMIT_WINDOW_MS: 60_000,
  NOT_FOUND_RATE_LIMIT_BLOCK_DURATION_MS: 600_000,
};
