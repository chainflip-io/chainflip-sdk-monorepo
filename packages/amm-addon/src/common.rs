...
pub fn fixed_point_to_power_as_fixed_point(x: U256, n: u32) -> U512 {
    let x = U512::from(x);
    ...
    (0..(32 - n.leading_zeros()))
        .zip(
            ...
            std::iter::once(x).chain(std::iter::repeat_with({
                let mut x = x;
                move || {
                    x = (x * x) >> 128;
                    x
...
pub fn nth_root_of_integer_as_fixed_point(x: U256, n: u32) -> U256 {
...
mod test {
    ...
    fn test_increase_sqrt_price() {
        fn inner<SD: SwapDirection>() {
            assert_eq!(
                SD::increase_sqrt_price(SD::WORST_SQRT_PRICE, 0),
                SD::WORST_SQRT_PRICE
            );
            assert_eq!(
                SD::increase_sqrt_price(SD::WORST_SQRT_PRICE, 1),
                SD::WORST_SQRT_PRICE
            );
    ...
    fn test_nth_root_of_integer_as_fixed_point() {
        fn fixed_point_to_float(x: U256) -> f64 {
            x.0.into_iter().fold(0.0f64, |acc, n| {
                (acc / 2.0f64.powi(64)) + (n as f64) * 2.0f64.powi(64)
            })
        }
    ...
    fn test_mul_div_floor() {
        ...
        assert_eq!(
            mul_div_floor(U256::MAX, U256::MAX - 1, U256::MAX),
            U256::MAX - 1
        );
    }
    ...
    fn test_mul_div() {
        assert_eq!(
            mul_div(U256::MAX, U256::MAX, U256::MAX),
            (U256::MAX, U256::MAX)
        );
        assert_eq!(
            mul_div(U256::MAX, U256::MAX - 1, U256::MAX),
            (U256::MAX - 1, U256::MAX - 1)
        );
