...
use crate::common::{
    ...
    tick_at_sqrt_price, Amount, BaseToQuote, LiquidityProvider, Pairs, PoolPairsMap, QuoteToBase,
    SetFeesError, SqrtPriceQ64F96, Tick, MAX_LP_FEE, MAX_TICK, MIN_TICK, ONE_IN_HUNDREDTH_PIPS,
    SQRT_PRICE_FRACTIONAL_BITS,
};
use primitive_types::{U256, U512};
use std::{collections::btree_map::BTreeMap, convert::Infallible, vec::Vec};
...
impl SwapDirection for BaseToQuote {
    ...
    fn next_liquidity_delta(
        ...
        if Self::further_liquidity(current_tick) {
            ...
            assert_eq!(
                current_tick,
                Self::current_tick_after_crossing_tick(MIN_TICK)
            );
            None
        }
    }
...
impl SwapDirection for QuoteToBase {
    ...
    fn next_liquidity_delta(
        ...
        if Self::further_liquidity(current_tick) {
            ...
            assert_eq!(
                current_tick,
                Self::current_tick_after_crossing_tick(MAX_TICK)
            );
            None
        }
    }
...
pub enum Size {
    ...
    Amount {
        maximum: PoolPairsMap<Amount>,
        minimum: PoolPairsMap<Amount>,
    },
}
...
impl PoolState {
    ...
    pub fn raw_current_sqrt_price(&self) -> SqrtPriceQ64F96 {
    ...
    pub fn collect_and_mint<T, E, TryDebit: FnOnce(PoolPairsMap<Amount>) -> Result<T, E>>(
        &mut self,
        ...
        if option_position.is_some() || minted_liquidity != 0 {
            ...
            lower_delta.liquidity_delta = lower_delta
                .liquidity_delta
            ...
            // Cannot overflow as due to liquidity_gross's MAX_TICK_GROSS_LIQUIDITY bound
            upper_delta.liquidity_delta = upper_delta
                .liquidity_delta
            ...
            self.positions
                .insert((lp.clone(), lower_tick, upper_tick), position);
    ...
    ) -> Result<(PoolPairsMap<Amount>, Liquidity, Collected, PositionInfo), PositionError<BurnError>>
    {
            ...
            .cloned()
        {
            ...
            if position.liquidity == 0 {
                ...
                *self
                    .positions
                    .get_mut(&(lp.clone(), lower_tick, upper_tick))
                    .unwrap() = position;
            };
    ...
    ///
    /// This function never panics
    pub fn required_asset_ratio<const ROUND_UP: bool>(
        &self,
        lower_tick: Tick,
        upper_tick: Tick,
    ...
    pub fn liquidity_to_amounts<const ROUND_UP: bool>(
        ...
        if liquidity > MAX_TICK_GROSS_LIQUIDITY {
            ...
            Ok(self
                .inner_liquidity_to_amounts::<ROUND_UP>(liquidity, lower_tick, upper_tick)
                .0)
        }
    }
        ...
        if self.current_tick < lower_tick {
            (
                PoolPairsMap::from_array([
                    (if ROUND_UP {
                        zero_amount_delta_ceil
                    } else {
                        zero_amount_delta_floor
                    })(
            ...
            (
                PoolPairsMap::from_array([
                    ...
                    (if ROUND_UP {
                        one_amount_delta_ceil
                    } else {
                        one_amount_delta_floor
                    })(
                        sqrt_price_at_tick(lower_tick),
                        self.current_sqrt_price,
                        liquidity,
    ...
    fn inner_amounts_to_liquidity(
        ...
        fn one_amount_to_liquidity(
            ...
            U256::full_mul(
                amounts[Pairs::Quote],
                U256::one() << SQRT_PRICE_FRACTIONAL_BITS,
    ...
    ///
    /// This function never panics
    pub fn liquidity(&self) -> Vec<(Tick, Liquidity)> {
    ...
    pub fn depth(
        ...
        if lower_tick <= upper_tick {
            let liquidity_at_lower_tick: Liquidity =
                self.liquidity_map
                    .range(..lower_tick)
                    .fold(0, |liquidity, (_, tick_delta)| {
                        liquidity
                            .checked_add_signed(tick_delta.liquidity_delta)
                            .unwrap()
                    });
            ...
            let (_liquidity, _tick, assets) = self
                ...
                .fold(
                    (
                        liquidity_at_lower_tick,
                        lower_tick,
                        PoolPairsMap::<Amount>::default(),
                    ),
                    |(liquidity, previous_tick, assets), (current_tick, liquidity_delta)| {
                        (
                            ...
                            *current_tick,
                            assets
                                + self
                                    .inner_liquidity_to_amounts::<false>(
                                        liquidity,
                                        previous_tick,
                                        *current_tick,
                                    )
...
fn one_amount_delta_floor(
    ...
    */
    mul_div_floor(
        liquidity.into(),
        to - from,
        U512::from(1) << SQRT_PRICE_FRACTIONAL_BITS,
    )
}
...
fn one_amount_delta_ceil(
    ...
    */
    mul_div_ceil(
        liquidity.into(),
        to - from,
        U512::from(1u32) << SQRT_PRICE_FRACTIONAL_BITS,
    )
}
